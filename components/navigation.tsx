    ...
    <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md bg-background/40 border-b border-border">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-8">
          ...
          <Button
            onClick={() => handleScroll("products")}
            className="bg-primary hover:bg-primary/90 text-xs md:text-sm px-2 md:px-4 py-1 md:py-2"
          >
            Order Now
          </Button>
          <button
          ...
          >
            ...
            {items.length > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 md:w-5 md:h-5 bg-pink-500 text-white text-xs font-bold rounded-full flex items-center justify-center">
                {items.length}
              </span>
            )}
          </button>
